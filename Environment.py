...
class RelaySelectionEnv(gym.Env):
    ...
    def __init__(self, rand_seed, is_train, eng):
        self.user_num_max = 5
        ...
        """     self.eng = matlab.engine.connect_matlab('meta')
                self.eng.addpath(self.eng.genpath("Matlab")) """

        ue_pos, rs_pos, matrix, rs_num, ue_num = self.eng.simulatorInit(self.rand_seed, nargout = 5)
        self.rs_num = int(rs_num)
        self.user_num = int(ue_num)
        if self.user_num > self.user_num_max:
            ValueError("self.user_num > self.user_num_max")
      
        self.user_positions = np.array(ue_pos)  # ue number *2
        self.rs_positions = np.array(rs_pos)  # rs number *2    

        # matrix 做一个补0操作
        matrix_padded = np.zeros((self.rs_num, self.user_num_max))
        matrix_padded[:, 0:self.user_num] = matrix
        
        self.relay_user_matrix = np.array(matrix_padded) # rs number * ue Maxnumber
        self.selection_matrix = np.zeros(self.rs_num) # rs number * (ue Maxnumber + 1)
    ...
    def reset(self, seed = None, options = None):
        ...
        self.sum_reward = 0
        self.sys_cap = 0
        self.ue_illegal = np.where(self.relay_user_matrix[self.current_rs,:] == 0)[0]
        self.ue_illegal = self.ue_illegal + 1
        self.user_mask = self.user_mask_0.copy()
        self.user_mask[self.ue_illegal] = 0
